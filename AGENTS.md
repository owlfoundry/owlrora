...
- `crates/owlrora-server/src/` — Axum server, `owlrora` executable, and embedded frontend delivery
...
- Keep product domain, gateway, protocols, infrastructure, HTTP, workers, bundled software encryption, the official CLI, and local stdio MCP adapter as modules compiled together in `owlrora-server`; do not create crates for them by default. The package installs one `owlrora` executable with server/client/MCP modes. CLI and MCP must use only public management HTTP APIs, preserve authorization/tenant/ETag/audit/one-time-secret semantics, and expose no generic raw HTTP tool. The only planned separate core package is the externally implementable `owlrora-key-provider` SPI, which must be published before `owlrora-server`. Published crate contents must build without Node.js or unpublished sibling paths.
