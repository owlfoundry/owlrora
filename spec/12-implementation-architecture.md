...
`cli-v<semver>` independently versions and publishes `owlrora-cli`. `server-v<semver>` versions the server family, creates a GitHub Release, publishes `owlrora-key-provider` before `owlrora-server`, and publishes one immutable versioned GHCR image without a mutable `latest` tag. CI owns package, platform, and image-smoke validation; release workflows construct and publish artifacts without repeating those checks. The server embeds production frontend assets and requires no unpublished sibling path; workspace development may use a local path override for the SPI, while packaged builds resolve its published exact dependency. The CLI has no dependency on either server-family crate. Checked server operation descriptors generate or validate committed package-local CLI bindings, and CI rejects drift; a shared contract crate is introduced only if real stable public types later justify it.
